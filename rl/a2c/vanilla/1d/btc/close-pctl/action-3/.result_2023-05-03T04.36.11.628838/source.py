...
FREQ = '1d'
LEVERAGE_LEVEL_N = 3
LEVERAGE_LEVELS = tuple(i/(LEVERAGE_LEVEL_N-1) for i in range(LEVERAGE_LEVEL_N))
...
def observe(my: Context[OhlcvWindow]) -> Obs:
    ...
    price_pctl = price_percentile(win)
    obs = np.array(price_pctl[-N_FEATURE:], dtype=np.float32)
    return obs


def act(my: Context[OhlcvWindow], action: Action) -> float:
    target_weight = LEVERAGE_LEVELS[int(action)]
...
class MyEnv(BasicTrEnv[Obs, Action]):
    ...
    action_space: Discrete = Discrete(LEVERAGE_LEVEL_N)
...
class MyAgent(A2CDiscreteAgent[Obs, Action]):
    ...
    def __init__(self) -> None:
        ...
        self.actor_critic_net = ActorCriticDiscrete(self.obs_dim, self.action_dim, **net_kwargs).to(self.device)
