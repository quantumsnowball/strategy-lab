...
def observe(my: Context[OhlcvWindow]) -> Obs:
    ...
    obs1 = price_pctl[-N_FEATURE:].to_numpy()
    obs2 = (my.portfolio.leverage, )
    obs = np.concatenate([obs1, obs2], dtype=np.float32)
    return obs
...
class MyEnv(BasicTrEnv[Obs, Action]):
    # Env
    observation_space: Box = Box(low=0, high=1, shape=(N_FEATURE+1, ), )
