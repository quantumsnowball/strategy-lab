...
LSTM_LAYERS_N = 1
LSTM_INPUT_SIZE = len(LNDIFF_LAG) + 1
LEVERAGE_LEVEL_N = 5
...
def observe(my: Context[OhlcvWindow]) -> Obs:
    ...
    # lndiff
    close_lndiff = np.stack([
        np.diff(close_ln, lag)[-N_FEATURE:]
        for lag in LNDIFF_LAG
    ], axis=1)
    # leverage
    obs_leverage = np.repeat(my.portfolio.leverage, N_FEATURE).reshape(-1, 1)
    # obs
    obs = np.hstack((close_lndiff, obs_leverage, )).astype(np.float32)
    return obs
