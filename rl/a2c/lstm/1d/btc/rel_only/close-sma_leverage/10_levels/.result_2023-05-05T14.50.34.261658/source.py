...
def observe(my: Context[OhlcvWindow]) -> Obs:
    ...
    # feat 1
    obs1 = normalize(close_sma_short)[-N_FEATURE:]
    # feat 2
    obs2 = normalize(close_sma_long)[-N_FEATURE:]
    # feat 3
    obs3 = normalize(close_sma_diff)[-N_FEATURE:]
    # feat 4
    obs4 = np.repeat(my.portfolio.leverage, len(obs1))
    # obs
    obs = np.stack((obs1, obs2, obs3, obs4), axis=1).astype(np.float32)
    return obs
