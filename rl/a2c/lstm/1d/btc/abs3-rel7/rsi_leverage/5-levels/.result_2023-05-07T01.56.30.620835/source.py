...
ABS_REWARD_W = 0.3
LSTM_LAYERS_N = 1
LSTM_INPUT_SIZE = len(RSI_LENGTH) + 1
LEVERAGE_LEVEL_N = 5
...
def observe(my: Context[OhlcvWindow]) -> Obs:
    ...
    # rsi
    rsi = np.stack([
        relative_strength_index(win, len=m)[-N_FEATURE:] / 100
        for m in RSI_LENGTH
    ], axis=1)
    # leverage
    lv = np.repeat(my.portfolio.leverage, N_FEATURE).reshape(-1, 1)
    # obs
    obs = np.hstack((rsi, lv)).astype(np.float32)
    return obs
