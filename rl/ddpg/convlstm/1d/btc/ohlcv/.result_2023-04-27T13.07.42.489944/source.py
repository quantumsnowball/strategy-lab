...
def observe(my: Context[OhlcvWindow]) -> Obs:
    ...
    obs = np.array(ohlcv_norm[-N_FEATURE:], dtype=np.float32)
    return obs
...
class MyAgent(DDPGAgent[Obs, Action]):
    ...
    def __init__(self) -> None:
        ...
        self.actor_net = ConvLSTM_DDPGActorNet(self.obs_dim, self.action_dim,
                                               ...
                                               conv1d_in_channels=5).to(self.device)
        self.actor_net_target = ConvLSTM_DDPGActorNet(self.obs_dim, self.action_dim,
                                                      min_action=self.min_action,
                                                      max_action=self.max_action,
                                                      conv1d_in_channels=5).to(self.device)
        self.critic_net = ConvLSTM_DDPGCriticNet(self.obs_dim, self.action_dim,
                                                 conv1d_in_channels=5).to(self.device)
        self.critic_net_target = ConvLSTM_DDPGCriticNet(self.obs_dim, self.action_dim,
                                                        conv1d_in_channels=5).to(self.device)
