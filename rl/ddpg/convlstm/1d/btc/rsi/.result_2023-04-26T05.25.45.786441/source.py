...
def observe(my: Context[OhlcvWindow]) -> Obs:
    ...
    rsi = relative_strength_index(win, len=RSI_LENGTH) / 100
    obs = np.array(rsi[-N_FEATURE:], dtype=np.float32)
    return obs
...
class MyAgent(DDPGAgent[Obs, Action]):
    ...
    def __init__(self) -> None:
        ...
        self.actor_net = ConvLSTM_DDPGActorNet(self.obs_dim, self.action_dim,
                                               min_action=self.min_action,
                                               max_action=self.max_action).to(self.device)
        self.actor_net_target = ConvLSTM_DDPGActorNet(self.obs_dim, self.action_dim,
                                                      min_action=self.min_action,
                                                      max_action=self.max_action).to(self.device)
        self.critic_net = ConvLSTM_DDPGCriticNet(self.obs_dim, self.action_dim).to(self.device)
        self.critic_net_target = ConvLSTM_DDPGCriticNet(self.obs_dim, self.action_dim).to(self.device)
...
def agent_step(my: Context[OhlcvWindow]):
    ...
    if my.count.beginning:
        ...
        # mark
        my.mark['rsi'] = relative_strength_index(my.event.win['Close'])[-1]
