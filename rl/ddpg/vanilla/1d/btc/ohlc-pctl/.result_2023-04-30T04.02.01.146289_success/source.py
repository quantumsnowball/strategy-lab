...
def observe(my: Context[OhlcvWindow]) -> Obs:
    ...
    obs = np.array(ohlc_pctl[-N_FEATURE:], dtype=np.float32)
    obs = obs.reshape(-1)
    return obs
...
class MyAgent(DDPGAgent[Obs, Action]):
    ...
    def __init__(self) -> None:
        ...
        self.actor_net = DDPGActorNet(self.obs_dim*4, self.action_dim,
                                      min_action=self.min_action,
                                      max_action=self.max_action).to(self.device)
        self.actor_net_target = DDPGActorNet(self.obs_dim*4, self.action_dim,
                                             min_action=self.min_action,
                                             max_action=self.max_action).to(self.device)
        self.critic_net = DDPGCriticNet(self.obs_dim*4, self.action_dim).to(self.device)
        self.critic_net_target = DDPGCriticNet(self.obs_dim*4, self.action_dim).to(self.device)
