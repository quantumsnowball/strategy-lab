...
class MyAgent(DDPGAgent[Obs, Action]):
    ...
    def __init__(self) -> None:
        ...
        self.actor_net = DDPGActorNet(self.obs_dim*5, self.action_dim,
                                      min_action=self.min_action,
                                      max_action=self.max_action,
                                      batch_norm=True).to(self.device)
        self.actor_net_target = DDPGActorNet(self.obs_dim*5, self.action_dim,
                                             min_action=self.min_action,
                                             max_action=self.max_action,
                                             batch_norm=True).to(self.device)
        self.critic_net = DDPGCriticNet(self.obs_dim*5, self.action_dim,
                                        batch_norm=True).to(self.device)
        self.critic_net_target = DDPGCriticNet(self.obs_dim*5, self.action_dim,
                                               batch_norm=True).to(self.device)
